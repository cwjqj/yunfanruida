--[[
============================================================
  R60BMP1 top-mount radar extended trajectory task

  Purpose: keep the V1.3 ROI people-count behavior and expose stable
  multi-person trajectories with path, direction, quality and events.
...
function
	local tname="RADAR_TOP_TRACK"
	local nid=1
	local pver,fver="Radar_Top_Track_2.0","FW_1.0.0"
	...
	function TrackExt.datapoint(t,now)
		if not t then
			return {uid=-1,radar_id=-1,state=0,state_text="none",x=0,y=0,height=0,speed=0,vx=0,vy=0,direction=0,direction_text="still",hits=0,moving_hits=0,max_disp=0,path_length=0,age_sec=0,lost_sec=0,quality=0,path=""}
		end
	...
	function TrackExt.refresh(all_tracks,now)
		...
		for i=1,math.min(#report_tracks,TrackExt.cfg.report_slots) do
			...
			parts[#parts+1]=table.concat({t.uid,t.rid,t.state,t.last_x or t.x,t.last_y or t.y,t.height,t.speed,t.vx or 0,t.vy or 0,direction,t.path_length or 0},",")
		end
	...
	while true do
		...
		local state_changed=(human_present~=last_human or motion~=last_motion or body_move~=last_body_move or has_person~=last_has_person or people_count~=last_people_count or target_count~=last_target_count or realtime_people_count~=last_realtime_people_count or accurate_people_count~=last_accurate_people_count or primary_target_id~=last_primary_target_id or traj_x~=last_traj_x or traj_y~=last_traj_y or traj_height~=last_traj_height or traj_speed~=last_traj_speed or traj_needdup==1)
