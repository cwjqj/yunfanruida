...
function
	...
	local function set_standard_people_count(candidate,source)
		...
		local cap=accurate_people_count
		if target_constraint_active==1 then
			cap=target_constraint_count
		end
		if target_constraint_active==1 and v>cap then
			v=cap
		elseif target_constraint_active~=1 and cap>0 and v>cap then
			v=cap
		end
		people_count=v
		people_min=v
		...
		log.info(tname,string.format("people standard=%d source=%s realtime=%d accurate=%d",people_count,tostring(source),realtime_people_count,accurate_people_count))
	end
	...
	local function try_parse_frame()
		...
		elseif control_word==0x82 then
			if (command_word==0x02 or command_word==0x82) and #data_bytes>=11 then
				...
				log.info(tname,string.format("traj raw=%d valid=%d dropped=%d ids=%s main=%d x=%d y=%d h=%d v=%d posture=%s in_bed=%d",frame_target_count,target_count,dropped_target_count,target_ids,primary_target_id,traj_x,traj_y,traj_height,traj_speed,posture_text,primary_in_bed_zone))
