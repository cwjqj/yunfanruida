...
function
	...
	local UNDER_BED_HEIGHT_MIN_CM=100
	...
	local UNDER_BED_SLOT_MATCH_DISTANCE_CM=70
	local UNDER_BED_SLOT_HOLD_SECONDS=300
	local UNDER_BED_EMPTY_CLEAR_SECONDS=20
	...
	local under_bed_slots={}
	local under_bed_empty_since=0
	...
	local function count_under_bed_slots(now)
		local n=0
		for i=#under_bed_slots,1,-1 do
			local slot=under_bed_slots[i]
			if now-slot.last_seen>UNDER_BED_SLOT_HOLD_SECONDS then
				table.remove(under_bed_slots,i)
			else
				n=n+1
			end
		end
		return clamp_under_bed_people_count(n)
	end

	local function sync_under_bed_people_count(now,source)
		local v=count_under_bed_slots(now)
		if v~=under_bed_people_count then
			under_bed_people_count=v
			log.info(tname,string.format("under bed people stable=%d source=%s",under_bed_people_count,tostring(source)))
			return true
		end
		return false
	end

	local function clear_under_bed_slots(source)
		under_bed_slots={}
		under_bed_empty_since=0
		if under_bed_people_count~=0 then
			under_bed_people_count=0
			log.info(tname,string.format("under bed people stable=0 source=%s",tostring(source)))
			return true
		end
		return false
	end

	local function remove_under_bed_slot_by_id(id)
		if id<0 then return false end
		for i=#under_bed_slots,1,-1 do
			if under_bed_slots[i].id==id then
				table.remove(under_bed_slots,i)
				return true
			end
		end
	...
	local function update_under_bed_slot(id,x,y,h,now)
		for i=1,#under_bed_slots do
			local slot=under_bed_slots[i]
			if slot.id==id and id>=0 then
				slot.x=math.floor((slot.x+x)/2)
				slot.y=math.floor((slot.y+y)/2)
				slot.height=h
				slot.last_seen=now
		...
		local best_index=0
		local best_dist=UNDER_BED_SLOT_MATCH_DISTANCE_CM+1
		for i=1,#under_bed_slots do
			local slot=under_bed_slots[i]
			local dx=x-slot.x
			local dy=y-slot.y
		...
		if best_index>0 and best_dist<=UNDER_BED_SLOT_MATCH_DISTANCE_CM then
			local slot=under_bed_slots[best_index]
			slot.x=math.floor((slot.x*2+x)/3)
			slot.y=math.floor((slot.y*2+y)/3)
			slot.height=h
			slot.last_seen=now
			if id>=0 then slot.id=id end
			slot.hits=slot.hits+1
		elseif #under_bed_slots<UNDER_BED_PEOPLE_COUNT_MAX then
			table.insert(under_bed_slots,{id=id,x=x,y=y,height=h,last_seen=now,hits=1})
		end
	end

	local function update_under_bed_slots_from_targets(targets,now)
		local seen=0
		local changed=false
		for i=1,#targets do
			local t=targets[i]
			if in_lower_side_area(t.x,t.y) and t.height>=UNDER_BED_HEIGHT_MIN_CM then
				update_under_bed_slot(t.id,t.x,t.y,t.height,now)
				seen=seen+1
			elseif remove_under_bed_slot_by_id(t.id) then
				changed=true
			end
		end
		if seen>0 then
			under_bed_empty_since=0
			return sync_under_bed_people_count(now,"xy_seen")
		end
		if changed then
			return sync_under_bed_people_count(now,"id_left_under_bed")
		end
		if people_count==0 and realtime_people_count==0 and accurate_people_count==0 and human_present==0 then
			if under_bed_empty_since==0 then under_bed_empty_since=now end
			if now-under_bed_empty_since>=UNDER_BED_EMPTY_CLEAR_SECONDS then
				return clear_under_bed_slots("empty_scene")
			end
		else
			sync_under_bed_people_count(now,"hold")
		end
		return false
	end
	...
	local function try_parse_frame()
		...
		elseif control_word==0x80 then
			if (command_word==0x01 or command_word==0x81) and #data_bytes>=1 then
				...
				if raw_human==1 then
					...
					update_under_bed_slots_from_targets({},os.time())
				end
		...
		elseif control_word==0x82 then
			if (command_word==0x02 or command_word==0x82) and #data_bytes>=11 then
				...
				update_under_bed_slots_from_targets(valid_targets,os.time())
	...
	while true do
		...
		local now=os.time()
		local state_changed=(human_present~=last_human or motion~=last_motion or people_min~=last_people_min or people_max~=last_people_max or realtime_people_count~=last_realtime_people_count or accurate_people_count~=last_accurate_people_count or target_count~=last_target_count or under_bed_people_count~=last_under_bed_people_count or posture_code~=last_posture_code)
