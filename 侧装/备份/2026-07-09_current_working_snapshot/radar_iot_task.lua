...
function
	...
	local SIDE_AREA_X_MIN_ABS=75
	local SIDE_AREA_X_MAX_ABS=165
	local BED_Y_MIN=0
	local BED_Y_MAX=500
	...
	local TRACK_PEOPLE_CONFIRM_FRAMES=2
	...
	local function apply_presence_from_stable_counts()
		if people_count>0 or target_count>0 then
			has_person=1
			human_present=1
		else
			has_person=0
	...
	local function accept_people_count(raw_count, raw_min, raw_max, confirm_frames, source)
		local candidate=clamp_people_count(raw_count)
		local needed=confirm_frames or PEOPLE_CHANGE_CONFIRM_FRAMES
		if candidate==0 then needed=PEOPLE_ZERO_CONFIRM_FRAMES end
		if candidate==people_count then
			people_pending_count=-1
			people_pending_hits=0
			if raw_min then people_min=clamp_people_count(raw_min) end
			if raw_max then people_max=clamp_people_count(raw_max) end
		...
		log.info(tname,string.format("people candidate=%d stable=%d hits=%d/%d source=%s",candidate,people_count,people_pending_hits,needed,tostring(source)))
		if people_pending_hits>=needed then
			people_count=candidate
			people_min=raw_min and clamp_people_count(raw_min) or candidate
			people_max=raw_max and clamp_people_count(raw_max) or candidate
			people_pending_count=-1
			people_pending_hits=0
			apply_presence_from_stable_counts()
			log.info(tname,string.format("people stable=%d source=%s",people_count,tostring(source)))
			return true
		end
		return false
	end
	...
	local function try_parse_frame()
		...
		elseif control_word==0x80 then
			if (command_word==0x01 or command_word==0x81) and #data_bytes>=1 then
				...
				if raw_human==1 then
					human_present=1
					has_person=1
					if people_count==0 and target_count==0 then
						accept_people_count(1,nil,nil,PEOPLE_CHANGE_CONFIRM_FRAMES,"human")
					end
				else
					if accept_people_count(0,0,0,PEOPLE_ZERO_CONFIRM_FRAMES,"human") then
		...
		elseif control_word==0x82 then
			if (command_word==0x02 or command_word==0x82) and #data_bytes>=11 then
				...
				if target_count>0 then
					has_person=1
					human_present=1
					accept_people_count(target_count,nil,nil,TRACK_PEOPLE_CONFIRM_FRAMES,"track")
				end
			...
			elseif (command_word==0x02 or command_word==0x82) and #data_bytes==0 then
				...
				if target_clear_hits>=TARGET_CLEAR_CONFIRM_FRAMES then
					clear_targets_stable()
					accept_people_count(0,0,0,PEOPLE_ZERO_CONFIRM_FRAMES,"track_empty")
				end
			end
		...
		elseif control_word==0x86 then
			if (command_word==0x0A or command_word==0x0C) and #data_bytes>=2 then
				...
				accept_people_count(raw_max,raw_min,raw_max,PEOPLE_CHANGE_CONFIRM_FRAMES,"people")
				log.info(tname,"people raw min="..raw_min.." max="..raw_max.." stable="..people_count)
			end
