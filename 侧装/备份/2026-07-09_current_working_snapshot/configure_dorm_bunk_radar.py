...
def parse_frame(frame):
    ...
    if control == 0x07 and command in (0x09, 0x89) and len(data) >= 9:
        return {
            "range_mode": data[0],
    ...
    if control == 0x07 and command in (0x11, 0x91) and len(data) >= 6:
        return {
            "tag_type": data[0],
...
def make_steps(args):
    ...
    steps.extend(
        ...
        (
            "干扰边无边界",
            build_frame(0x07, 0x1A, bytes([args.edge_mode])),
            build_frame(0x07, 0x9A, b"\x0F"),
...
def main():
    arg_parser = argparse.ArgumentParser(
        description=(
            ...
            "using angled wall installation."
        )
    )
    ...
    arg_parser.add_argument("--angle-z-deg", type=float, default=30.0)
    arg_parser.add_argument("--height-cm", type=int, default=220)
    ...
    arg_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also send fixed detection boundary settings. Disabled by default "
            "because 0x07 range/tag settings can suppress trajectory frames."
        ),
    )
    arg_parser.add_argument(
        "--edge-mode",
        ...
        default=4,
        choices=(1, 2, 3, 4),
        help="Default 4 means no interference edge boundary.",
    )
    ...
    arg_parser.add_argument("--track-distance-threshold-cm", type=int, default=100)
    ...
    print("Dorm bunk-bed angled-installation radar configuration")
    print(f"Port: {args.port}, baud: {args.baud}")
    print(
        "Room boundary: "
        f"X+={args.x_pos_cm}cm, X-={args.x_neg_cm}cm, "
        f"Y+={args.y_pos_cm}cm, Y-={args.y_neg_cm}cm "
        f"({'will be written' if args.set_fixed_range else 'not written'})"
    )
    print(
        "Mounting: "
