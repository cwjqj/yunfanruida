...
function
	...
	local nid=1
	local pver,fver="Radar_Top_Posture_3.1","FW_1.0.0"
	...
	local AISLE_ABS_X_MAX_CM=75
	local BED_DESK_ABS_X_MAX_CM=165
	-- Set this only after measuring the upper-bed surface from the floor.
	-- Zero means uncalibrated and activates conservative ambiguity protection.
	local BED_SURFACE_HEIGHT_CM=0
	local BED_LYING_REL_MIN_CM=15
	local BED_LYING_REL_MAX_CM=70
	local ZONE_OUTSIDE=0
	local ZONE_AISLE=1
	local ZONE_BED_DESK=2
	local TrackExt={
		cfg={
			report_slots=3,
			path_points=12,
	...
	local Posture={
		report={standing=0,sitting=0,lying=0,unknown=0,aisle=0,bed_desk=0,event="",event_seq=0}
	}
	local Count={
		cfg={
	...
	local function posture_zone(x,y)
		if x<AREA_X_MIN or x>AREA_X_MAX or y<AREA_Y_MIN or y>AREA_Y_MAX then
			return ZONE_OUTSIDE,"outside"
		end
		local ax=math.abs(x)
		if ax<AISLE_ABS_X_MAX_CM then return ZONE_AISLE,"aisle" end
		if ax<=BED_DESK_ABS_X_MAX_CM then return ZONE_BED_DESK,"bed_desk" end
		return ZONE_OUTSIDE,"outside"
	end

	local function bed_profile_ready()
		return BED_SURFACE_HEIGHT_CM>0
	end
	...
	function Posture.classify(height,current,zone)
		if zone==ZONE_BED_DESK and bed_profile_ready() then
			local relative=height-BED_SURFACE_HEIGHT_CM
			if relative>=BED_LYING_REL_MIN_CM and relative<=BED_LYING_REL_MAX_CM then
	...
	function Posture.update(t,now)
		...
		t.posture_zone,t.posture_zone_text=posture_zone(t.last_x or t.x,t.last_y or t.y)
		...
		local observed=Posture.classify(t.posture_height,stable,t.posture_zone)
	...
	function Posture.valid(t,now)
		return t and (t.posture_code or POSTURE_UNKNOWN)~=POSTURE_UNKNOWN
			and t.state=="active"
			and not ((t.posture_zone or ZONE_OUTSIDE)==ZONE_BED_DESK and not bed_profile_ready() and (t.posture_height or 0)>=145)
			and now-(t.posture_observed_at or 0)<=POSTURE_VALID_HOLD_SEC
	end

	function Posture.datapoint(t,now)
		if not t then
			return {uid=-1,posture=0,posture_text="unknown",valid=0,confidence=0,height=0,height_valid=0,reason="no_track",zone=0,zone_text="outside",in_bed_zone=0,bed_profile_ready=bed_profile_ready() and 1 or 0,relative_height=0,candidate=0,candidate_text="unknown",age_sec=0,observed_age_sec=-1}
		end
		...
		elseif (t.posture_zone or ZONE_OUTSIDE)==ZONE_BED_DESK and not bed_profile_ready() and (t.posture_height or 0)>=145 then reason="bed_profile_unconfigured"
		...
		return {
			...
			zone=t.posture_zone or ZONE_OUTSIDE,zone_text=t.posture_zone_text or "outside",
			in_bed_zone=((t.posture_zone or ZONE_OUTSIDE)==ZONE_BED_DESK) and 1 or 0,
			bed_profile_ready=bed_profile_ready() and 1 or 0,
			relative_height=bed_profile_ready() and ((t.posture_height or 0)-BED_SURFACE_HEIGHT_CM) or 0,
	...
	function TrackExt.datapoint(t,now)
		...
		return {
			...
			posture_zone=posture.zone,posture_zone_text=posture.zone_text,in_bed_zone=posture.in_bed_zone,
			bed_profile_ready=posture.bed_profile_ready,posture_relative_height=posture.relative_height,
	...
	function TrackExt.refresh(all_tracks,now)
		...
		Posture.report.aisle=0
		Posture.report.bed_desk=0
		for i=1,#report_tracks do
			local t=report_tracks[i]
			if (t.posture_zone or ZONE_OUTSIDE)==ZONE_AISLE then Posture.report.aisle=Posture.report.aisle+1
			elseif (t.posture_zone or ZONE_OUTSIDE)==ZONE_BED_DESK then Posture.report.bed_desk=Posture.report.bed_desk+1 end
	...
	local function target_datapoint(t)
		...
		local zone,zone_text=posture_zone(t.x or 0,t.y or 0)
		return {
			...
			in_bed_zone=(has_target and zone==ZONE_BED_DESK) and 1 or 0,
			zone=has_target and zone or ZONE_OUTSIDE,
			zone_text=has_target and zone_text or "outside"
		}
	end
	...
	local function fill_param(dst,names,force_all)
		...
		if force_all or param_wanted(names,"aisle_track_count") then dst.aisle_track_count=Posture.report.aisle end
		if force_all or param_wanted(names,"bed_desk_track_count") then dst.bed_desk_track_count=Posture.report.bed_desk end
		if force_all or param_wanted(names,"bed_profile_ready") then dst.bed_profile_ready=bed_profile_ready() and 1 or 0 end
		if force_all or param_wanted(names,"bed_surface_height") then dst.bed_surface_height=BED_SURFACE_HEIGHT_CM end
